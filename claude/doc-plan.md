...
**2026-07-30.** Phase 1 of the remaining articles was #12 (summary view) and #19 (model scope). The
summary view is **demoted to phase 2** — it is four labels and two spans, and it needs a screenshot
nobody has taken yet. #19 is **written**, and written against the engine source rather than from memory:
the Miller stability formula, Litz spin drift (`1.25 × (Sg + 1.2)`) and aerodynamic jump (AB Eq 5.4), the
split of Coriolis into a latitude-only horizontal term and an azimuth-dependent Eötvös vertical one, the
constant 9.80665 gravity, wind as a horizontal vector only, Sg computed once at the muzzle, RK2 with the
output step halved-then-decimated internally, and the 50 ft/s / 10,000 ft stop conditions. The
application never sets `Integrator` or `MaximumCalculationStepSize`, so the engine defaults are what
users get.
...
| 6 | **The Rifle tab** (`rifle-tab.md`) — **written 2026-07-28** | Sight height as geometry rather than a detail (line of sight crossing the bore, where the near zero lands, measured centre of bore to centre of tube) and the one field the tab cannot do without; presets from `data/dictionaries.xml`, including that a sight preset **overwrites the Zero tab's distance** so it must be picked first, and `Tools → Edit Sights/Barrels` writing back to that file; what click values buy — the two *Clicks* columns (**n/a** without them) and dialled clicks on Parameters — entered in the turret's own unit, unrelated to `View → Angular Units`; and the twist, whose direction decides which way spin drift goes and which is only **one of three** inputs it needs, the others being bullet diameter and length | 1 |
| 7 | **The Zero tab** (`zero-tab.md`) — **written 2026-07-28**, absorbing "Zeroing" | Built on the idea that makes the tab legible: the app never asks for a sight angle, it **describes the zeroing event** and solves for the angle. Two scenarios, each with *when* it is needed and not only how. (1) The ordinary zero — one number — plus what "leave the rest alone" actually assumes: same load, **same atmosphere as the shot** (not standard), no wind, level. (2) The zero established under other conditions, group by group, ranked by how much each matters: different load (always — feet at distance; zero supersonic, shoot subsonic is the worked case), deliberate offset zero (always, a fixed bias), a crosswind dialled out (if real), other atmosphere (only for long zero distances), zero shot angle (rare). Signs stated: V offset positive up, H offset positive left. Documents F-4 as a trap | 1 | Understand that the app computes the zero mathematically rather than being told a sight angle, then use zero distance, impact offset at zero, and — the feature no other free solver has — zeroing with a *different* cartridge, atmosphere or wind than the shot. Worked case: zero supersonic, shoot subsonic | 1 |
| 4 | **The Weather tab** (`weather-tab.md`) — **written 2026-07-28** | Describe the air without the error that ruins a long-range solution: the pressure field is **station** pressure, and every published figure (altimeter setting, weather app) is sea-level-corrected — ~29.92 vs ~24.9 inHg at 5,000 ft, i.e. air 20 % too dense. Also that all four fields are required together, what *Reset to Standard* writes (29.92 inHg / 1013.2 hPa, 59 °F, 78 %), that temperature moves the Mach column as well as the density, and that propellant temperature sensitivity is **not** modelled | 1 |
| 5 | **The Wind tab** (`wind-tab.md`) — **written 2026-07-28** | Enter wind so the windage number means something: direction clockwise from downrange (0 tailwind, 90 from the right, 180 headwind, 270 from the left), read off the dial rather than the number, relative to the line of fire and **not** the meteorological "wind from" convention; each row's wind acting from its own distance to the next row's; how to make a wind stop (a zone with velocity 0); and why zones rarely earn their keep — deflection is proportional to the time left, so the wind at your position dominates. Notes that aerodynamic jump reads the first zone only | 1 |
| 8 | **The Parameters tab** (`parameters-tab.md`) — **written 2026-07-28**, absorbing "Range, step, angle and the Coriolis effect"; promoted to Phase 1 as the last tab article | The step is **presentation, not precision** — the solver halves it and divides down to ~0.1–0.5 m internally, so a fine step buys rows, not accuracy or wall-clock; the reticle and summary ignore it entirely (own 2.5 m trajectory); a table can end early because the engine abandons the shot below 50 ft/s or past 10,000 ft of drop. Shot angle is this shot's, not the Zero tab's. Dialled clicks are counts needing a click size from the Rifle tab, else silently discarded (F-5). Coriolis given as two effects with **measured** magnitudes (.223 69 gr, 45°): horizontal depends on latitude only — 1.1 in at 600 yd, 3.8 in at 1,000, right in the north; vertical depends on azimuth — ±4.9 in at 1,000, nil due north/south; latitude alone is the honest answer when the bearing is unknown | 1 | Know what each run-time knob changes: max range and step (and the cost of a fine step), uphill/downhill shots, already-dialled clicks, and the azimuth + latitude that Coriolis needs. Says when Coriolis is noise and when it is not | 2 |
| 9 | **Reading the table** (`reading-the-table.md`) — **written 2026-07-28** | Every column with its units (metric drop/windage are **cm**, not mm), the n/a row at the muzzle, and the two conventions. Drop vs Hold as a distance vs an angle. `View → Drop`: **the two settings are identical on a level shot** and diverge only on an incline — measured, .223 69 gr at 500 yd, 30° up: −55.91 in over line of sight against **+8,936 in** over muzzle level, because the target is 750 ft above you. Windage is signed **positive = left** and is the *total* of wind, spin drift, Coriolis and any H offset at zero, which is why a no-wind table can show windage. O.G.W. given with its formula (weight² × velocity³ × 1.5e-12) so the reader can see it knows nothing about bullet construction | 1 | Interpret every column and the two conventions behind them — drop over line of sight vs over muzzle level, hold vs drop, clicks in the reader's chosen angular unit, windage adjustment, energy, optimal game weight | 1 |
| 10 | **The chart view** (`chart-view.md`) — **written 2026-07-28**, split out of "Chart, reticle and summary" | The five variables `View → Chart` offers; the mode worth knowing — Drop + *Over Muzzle Level* draws **two curves**, bullet path and line of sight, the textbook picture the table cannot show, where the gap between them *is* the hold; `Zoom Y Axis to Visible Range` (`Ctrl+Shift+Z`) as the fix for the flat-line-at-the-bottom problem after an X zoom; when a legend appears (two series, or a comparison) | 1 |
| 11 | **The reticle view** (`reticle-view.md`) — **written 2026-07-28**, split out likewise | Loading a reticle (`Mil-Dot` or the shipped set in `data/reticle`); the three overlays — Far BDC and Near BDC labelling the marks in yards/metres from a **fine trajectory independent of the table step**, and Target drawing a box to scale *behind* the reticle with its angular size printed; moving-target lead as a dashed aim-off box plus a `Lead: … left/right` readout that needs no sign interpretation. States the assumption nobody documents: the picture is drawn at the reticle's nominal subtension, true for FFP at any magnification and for SFP only at its design magnification | 1 |
| 12 | **The summary view** — **moved to phase 2 (2026-07-30)**: the view is four labels and a span, too simplistic to lead with | Point-blank range and the dead-zone span, near and far zero, and the distance where the bullet goes subsonic. The one view of the four with no article yet, and the one with no screenshot either | 1 | Use the other three views: chart variables (velocity, Mach, drop, windage, energy) and Y-axis zoom; the sight picture with far/near BDC and target overlays, a loaded custom reticle, and the moving-target aim-off box; the summary's point-blank range, dead zone, near and far zero, and the distance where the bullet goes subsonic | 1 |
| 13 | **Comparing loads** (`comparing-loads.md`) — **written 2026-07-28**, split from "Comparing loads, saving and exporting" | Building a comparison from open windows and the menu asymmetry that decides which window you must be looking at (**Add** needs a trajectory window, **Remove Last Added** needs the Compare window); series named from the ammunition, so unnamed loads all become `Trajectory`; **it is a snapshot, not a live link** — a recomputed source does not move its curve, which turns into the before/after trick of adding the same window twice around an edit; mixed units restated in the Compare window's own system; and what is worth comparing per chart variable, with the warning to change one variable at a time | 2 |
| 14 | **Saving and exporting** | `.trajectory` files (what they carry: the shot plus the window's measurement and angular units), and the two CSV formats — local for Excel, invariant for scripts and for sending on. Partly covered already at the end of `reading-the-table.md` | 2 | Put several trajectories on one chart, save and reopen a shot, and export CSV in the local (Excel) or invariant (portable) format — including why that choice exists | 2 |
...
| # | Article | Goal | Phase |
|---|---|---|---|
| 17 | **Choosing a drag model** (`choosing-a-drag-model.md`) — **written 2026-07-28** | What a BC actually is — a ratio to a reference *shape*, constant only if the shapes match, which they do not — and why that makes G1 a muzzle-band average. The four ways to describe drag here, in ascending order of what they know about the bullet. The form-factor switch and why a form factor of 1 on GC is the measured-table convention. And the proportion: a single BC is plenty inside 300, the model shows from 500-800, it dominates past 800; the muzzle velocity you measured yourself still matters more | 1 | Decide between a standard curve and a measured one, knowing what a BC actually is (a ratio to a reference projectile, not a property of the bullet), when G1 misleads and G7 does not, and what the form-factor switch means | 1 |
| 18 | **Custom drag tables** (`custom-drag-tables.md`) — **written 2026-07-28** | Where `.drg` files come from; the five things `Browse…` does and why muzzle velocity is deliberately not one; what the header carries (weight and diameter are **part of the drag model**, not decoration — the curve is stored scaled by sectional density, so a header with no diameter cannot be computed) and that pre-1.1.11.2 files store length/source as `0`; how the reference travels — stored by file **name**, re-found under `data/drg` and subfolders, read at calculation time so an edited file changes the next answer, and unresolvable means the shot fails; and that `Clear` resets to `0.5 G1` as an implausible placeholder | 1 | Load a projectile's own measured Cd curve, understand the form-factor-of-1 convention that comes with it, and know what a `.drg` does *not* carry — the muzzle velocity is the reader's to enter | 1 |
| 19 | **Approximating a drag table** (`approximating-a-drag-table.md`) — **written 2026-07-28** | Which of the two dialogs the reader's data chooses for them; the shared header requirements (name, **weight and diameter mandatory** because they set the scale); knots keyed by **Mach** not velocity, each remembering its own table and converted to the base table at its own Mach on save; the velocities dialog's stricter rules — ≥3 readings, distinct distances, **velocity must decrease** — and that *Set Atmosphere* drives the recovered coefficients, so leaving it standard for your own high-altitude session is wrong. Ends with how far to trust the result: better than a single BC over the span the data covers, worst below transonic where the data usually stops, and unable to invent information | 1 | Produce a usable curve from what a data sheet or a chronograph actually gives: a multi-BC curve, or measured downrange velocities. Both tools in one article because the choice between them is the reader's real question. Must state the accuracy caveat next to the Save button, not below the fold | 1 |
| 20 | **Converting a ballistic coefficient** (`converting-a-bc.md`) — **written 2026-07-28** | No Convert button — the answer follows the inputs; the ratio it computes; why the reference velocity is **part of the answer** rather than a setting, with the accuracy from the code's own note (~1% of published pairs between Mach 1.8-2.5, ~9% low near Mach 1.3, warned below Mach 1.5); how to choose a reference (the band you shoot in, not the muzzle); and three cases where something better exists — published pairs, a multi-BC curve, a measured `.drg` | 2 | Answer the everyday G1 ↔ G7 question at a chosen reference velocity, and understand why the answer is velocity-dependent — i.e. why a single converted number is a compromise | 2 |
...
| # | Article | Goal | Phase |
|---|---|---|---|
| 22 | **Hit probability** (`hit-probability.md`) — **written 2026-07-28** | The Monte Carlo in one paragraph, then the inputs with the one that decides everything spelled out from the library's own guidance: **Group (1σ) is a per-axis standard deviation**, the best of up to ten supported shots, ~4 MOA ordinary and ~1 MOA precision, and **extreme spread is about four times it** — so a 1 MOA extreme spread typed in as 1 MOA claims four times the shooter. The position multipliers as shipped (Supported 1/1, Prone 2/2, Kneeling 4/3, Standing 5/4); range and wind error being the shooter's while MV deviation is the ammunition's; shots 1,000-50,000 (noise ±1.3% at 1,000) and the seed as a way to see that noise. The three outputs, including reading the **shape** of the scatter as the diagnosis and that the plot is thinned to 2,000 markers while the probability uses all of them. Then the section that matters most — what the estimate assumes you got right: the hold is correct for the range/wind you *estimated*, **dialled clicks are deliberately ignored** so the hold is not counted twice, the vital zone is a circle, errors are independent and normal so the tails are optimistic, and nothing models the rifle changing. Closes by reading the screenshot: 18.3% with 4/7/12/15/20 says the shot is not really available | 1 | Build an honest error budget — group size as a **1σ per-axis** figure (about a quarter of a large group's extreme spread), shooting position, range and wind estimation error, muzzle-velocity deviation — then read the three outputs: single-shot probability, shots for a first hit at 50–98 %, and the impact scatter. Must be explicit that the estimate assumes a correct come-up and wind hold, and a circular vital zone | 1 |
...
| # | Article | Goal | Phase |
|---|---|---|---|
| 21 | **Ammunition library and presets** (`library-and-presets.md`) — **written 2026-07-28** | That there is no library window — a saved load is a **file**, and the library is whichever folder holds them; `.ammox` and legacy `.ammo`; the two dictionary editors, both writing the whole of `data/dictionaries.xml`, which a release replaces (keep a copy outside `data`); the sight preset's **Default Zero** overwriting the Zero tab; twist rate *and* direction both mattering; and that the dictionary editors are standalone, so they open in the last-created trajectory's units | 2 | Stop re-typing: save loads, build sight presets with click values, and barrel presets with twist | 2 |
| 16 | **The reticle editor** (`reticle-editor.md`) — **written 2026-07-28** | What the separate application is for and why a wrong-subtension reticle makes the whole sight picture worthless; what it deliberately is not (no freehand, drag, zoom or undo — a reticle is a measuring instrument, so coordinates are typed); the window, the View menu's three items, the `.reticle` files in `data/reticle`; and F-6, that nothing warns about unsaved changes | 2 |
| 16b | **Reticle size and zero** (`reticle-parameters.md`) — **written 2026-07-28** | The coordinate space, which is two conventions in one dialog and the thing to read twice: **Size** is a rectangle of sky, **Zero** places the aiming point in it measured from the **top-left**, and **element coordinates are then measured from the zero with Y positive up**. Choosing a size (proportion, not on-screen size; off-centre zeros for BDC work; the M16's 350 × 350 moa); the status bar as the measuring tool given there is no dragging; and that the element buttons stay disabled until the size is non-zero, which enforces parameters-then-elements | 2 |
| 16c | **Reticle elements** (`reticle-elements.md`) — **written 2026-07-28** | The six operations, that **Duplicate lands exactly on top of the original** (so edit it immediately — and that this is how a row of marks gets built), and that **list order is draw order**, which makes Up/Down a drawing tool rather than tidying. Field lists for Line, Circle, Rectangle and Text (anchor is what lines a column of labels up), and the key distinction: **a BDC point is not a drawing** — the editor labels it with its own Y because it has no trajectory, while the calculator labels it with a range | 2 |
| 16d | **Reticle paths** (`reticle-paths.md`) — **written 2026-07-28** | The path dialog as an editor within the editor; **Revert as the undo** it needs because the dialog edits the element in place (and Cancel reverts too); Move To / Line To / Arc as pen movements, with a mid-path Move To starting a disconnected sub-shape; the arc's four-way ambiguity resolved by Clockwise + Major Arc, and the radius ≥ half-chord limit; order being the shape itself; fill closing the region implicitly; and a worked three-entry chevron | 2 | Build or edit a reticle: the moa/mil coordinate space and where zero sits in it, the element list (lines, paths, circles, rectangles, text, BDC marks), and how the result appears in the sight picture | 2 |
...
| 19 | **What the model includes** (`what-the-model-includes.md`) — **written 2026-07-30** | Judge the numbers: 3DOF point-mass integration plus spin drift, aerodynamic jump and Coriolis; what a 4DOF model adds (angular motion, not a better drag model); and the risk notice in full rather than as fine print | 1 |
