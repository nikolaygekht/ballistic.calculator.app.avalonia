...
- **`y` is vertical, not perpendicular to the line of sight.** Gravity is a clean `−g ŷ` and needs no
...
- **`x` is horizontal, not along the line of sight.** So the line-of-sight distance a row reports is
  `x / cos(α)`, where `α` is the shot angle.
...
velocity `V₀` pointed along the bore:
...
- `w` — the wind vector. Drag depends on motion through the *air*, so wind enters by shifting the
...
`|v| = √(vx² + vy² + vz²)` — the ground-relative speed, not the air-relative one. The distinction
matters in a headwind: `v_a` is what the drag law and the Mach number use, `v` is what the table shows
...
with `w` in grains and the velocity in ft/s for the last one. Note the reported Mach uses `|v|`, while
the drag lookup inside the step uses `|v_a|`; in calm air they are identical.
...
Wind is a **horizontal** vector, given as a speed `W` and a direction `φ` (the
...
and then rotated into the shooting frame by the sight incline `θ` and the cant angle `κ`:
...
Wind acts on the trajectory through **one mechanism only**: it changes `v_a`, and therefore the
...
as the bullet slows. `Δt` is quantized to a `TimeSpan` tick (100 ns) and floored there.

`Δs` comes from the requested output step, not from the user's step directly — see
...
report — and since `Δt = Δs/v_x`, letting it continue would inflate the step without limit.
...
`h·(cos α − 1)` term is intentional, and pins the muzzle row to exactly `−h`. Both conventions are
...
`x·tan α`, and the line of departure (the bore line) `x·tan θ − h`. The chart's two-curve mode draws the
bullet crossing them.

## Zeroing is the same model, solved backwards

The launch angle `θ_zero` is not given, it is found. The engine runs the **full** trajectory to the zero
