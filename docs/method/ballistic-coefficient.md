...
- $A$ — the projectile's frontal area, $π d^2 / 4$ for a bullet of diameter $d$
...
Substituting `d²/m = 1/SD` and `C_d,bullet = i · C_d,table` back into the acceleration:
...
dimensionless ratio to that standard — the **density factor** `ρ/ρ₀`, which the
...
velocity `v_a = v − w`: it is `|v_a| · v_a` that carries both the `v²` magnitude and the direction.
...
- **The Mach number is local.** `M = |v_a| / c`, where `c` is the speed of sound at the bullet's
...
| **Form factor** | the shape factor `i`, e.g. `1.05 G7` | `BC = SD / i`, from the bullet's weight and diameter |
...
From `BC = SD/i`, sectional density is a property of the bullet and cancels:
...
the G7 standard is a long boat-tailed shape that a modern match bullet resembles closely, so `i_G7`
lands near 1; the G1 standard is a blunt flat-based shape that it does not resemble at all, so `i_G1`
...
The whole construction rests on `C_d,bullet(M) = i · C_d,table(M)` — one constant multiplier at every
...
`BC(M)` is interpolated linearly between the supplied knots and held flat beyond the end ones. The
...
Because `BC = SD/i` and the two `i` values belong to different reference curves, converting a G1 BC to
...
- Drag acceleration is `(π/8)·ρ·v²·C_d(M)/BC`. The BC is a divisor of drag, nothing more.
- `BC = SD/i`: sectional density carries the mass and the calibre, the form factor carries the shape.
